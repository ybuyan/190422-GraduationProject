<template>
     <div>
        ...
        <mu-container>
            <mu-list>
                <mu-sub-header>logic</mu-sub-header>
...
export default {
   data() {
       return {
           ...
           data:JSON.parse(localStorage.getItem("logicData"))
       }
   },
   ...
   methods: {
        getScore() {
           let apiUrl = this.$store.state.apiUrl+"game/findLogicResult";
           axios({
               method: "post",
               url: apiUrl,
               data:{
               ...
               let logicData = JSON.stringify(res.data)
               localStorage.setItem("logicData",logicData)
